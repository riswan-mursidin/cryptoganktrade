...
require_once "viewmember.php";
require_once "edituser.php";
$memberview = new viewMember();
$memberData = new getAllMember();
require_once "confirmaction.php"
?>
<!DOCTYPE html>
<html lang="en">
	<head>
	...
	<body>
		<div class="container">
			<div class="row">
				<h5 class="text-center text-white mt-5 mb-3" style="font-size: smaller">CONFIRM</h5>
				
				<div class="card-balance col-6 text-center">
					
						<div class="form-balance" style="cursor: pointer;" id="wdshow">
							WITHDRAW
							<span class="ms-1"> </span>
						</div>
				</div>
				<div class="card-balance col-6 text-center">
					<div class="form-balance" style="cursor: pointer;" id="deposhow">
						DEPOSTE
						<span class="ms-1"> </span>
					</div>
				</div>
				<div class="col-12 mt-3" id="wd">
					<table class="table table-dark table-sm table-striped table-responsive text-center" style="font-size: smaller">
						...
						<tbody class="withdraw" style="font-size: smaller; display: none">
							<?= $memberview->getConfirWD() ?>
						</tbody>
					</table>
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-end wdlist">
						</ul>
					</nav>
				</div>
				<div class="col-12 mt-3" id="depo" style="display: none;">
					<table class="table table-dark table-sm table-striped table-responsive text-center" style="font-size: smaller">
						<thead>
							<tr>
								<th scope="col">Date</th>
						...
						<tbody class="deposit" style="font-size: smaller; display: none;">
						<?= $memberview->getConfirDEPO() ?>
						</tbody>
					</table>
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-end depolist">
						</ul>
					</nav>
				</div>
			</div>
		...
		<?php  
		$getwd = $memberData->getWithDraw(null, null, "all");
		foreach($getwd as $wd){
		?>
			<div class="modal fade" id="wdd<?= $wd['id_history'] ?>"  data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-scrollable">
					<form action="" method="post" class="modal-content" style="background-color: rgb(48, 48, 48)">
						<div class="modal-header text-white">Confirm Withdraw</div>
							<div class="modal-body text-white">
								<div class="mb-3">
									<label for="" class="form-label">Wallet <?= $wd['type_coin_wd_history'] ?></label>								
									<p id="wallet<?= $wd['id_history'] ?>" style="font-weight: bolder"><?= $wd['token_history'] ?></p>
									<button type="button" class="btn btn-outline-secondary copy<?= $wd['id_history'] ?>">
										Copy
									</button>
								</div>
								<div class="mb-3">
									<label for="" class="form-label">Amount</label>								
									<input type="text" name="saldo" readonly value="<?= $wd['jum_history'] ?>" class="form-control">
								</div>
							</div>
						<div class="modal-footer">
							<input type="hidden" name="type_wd" value="<?= $wd['type_wd_history'] ?>">
							<input type="hidden" name="admindepo" value="<?= $wd['biaya_persen'] ?>">
							<input type="hidden" name="id_history" value="<?= $wd['id_history'] ?>">
							<input type="hidden" name="username" value="<?= $wd['username_history'] ?>">
							<input type="hidden" name="coin" value="<?= $wd['type_coin_wd_history'] ?>">
							<button type="submit" name="tolak_wd" <?= $wd['status_history'] != "Pending" ? "disabled" : "" ?> class="btn btn-danger btn-sm">Rejected</button>
							<button type="submit" name="confirm_wd" <?= $wd['status_history'] != "Pending" ? "disabled" : "" ?> class="btn btn-success btn-sm" >Accept</button>
						</div>
					</form>
				</div>
			</div>
			...
			<script>
				var clipboard = new ClipboardJS('.copy<?= $wd['id_history'] ?>', {
					target: function () {
						return document.querySelector("#wallet<?= $wd['id_history'] ?>");
					},
				});
		...
		$getdepo = $memberData->getDeposit(null, null, "all");
        foreach($getdepo as $depo){
		?>
			<div class="modal fade" id="depoo<?= $depo['id_history'] ?>"  data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-scrollable">
					<form action="" method="post" class="modal-content" style="background-color: rgb(48, 48, 48)">
						<div class="modal-header text-white">Confirm Withdraw</div>
							<div class="modal-body text-white">
								<div class="mb-3">
									<label for="" class="form-label">Amount</label>								
									<input type="text" name="saldo" readonly value="<?= $depo['jum_history']?>" class="form-control">
								</div>
							</div>
						<div class="modal-footer">
							<input type="hidden" name="id_history" value="<?= $depo['id_history'] ?>">
							<input type="hidden" name="admindepo" value="<?= $depo['biaya_persen'] ?>">
							<input type="hidden" name="username" value="<?= $depo['username_history'] ?>">
							<input type="hidden" name="coin" value="<?= $depo['type_coin_history'] ?>">
							<button type="submit" name="tolak_depo" <?= $depo['status_history'] != "Pending" ? "disabled" : "" ?> class="btn btn-danger btn-sm">Rejected</button>
							<button type="submit" name="confirm_depo" <?= $depo['status_history'] != "Pending" ? "disabled" : "" ?> class="btn btn-success btn-sm" >Accept</button>
						</div>
					</form>
				</div>
			</div>
		...
		<script src="js/pagedepowd.js"></script>
		<script src="js/viewcoin2.js"></script>
