...
<html lang="en">
	...
	<body>
		<div class="container">
			<div class="row">
				<h5 class="text-center text-white mt-5 mb-3" style="font-size: smaller">DATA MEMBER</h5>
				<div class="container">
					<div class="row">
						<div class="col-4 col-sm-4">
							<input type="text" name="searchmember" id="search_member" class="form-control-sm mb-2" placeholder="type to seacrh..">
						</div>
					</div>
					<div class="table-responsive mb-2">
						<table class="table table-dark table-sm table-striped table-responsive text-center data-search" style="font-size: smaller">
							<thead>
								<tr>
							...
							<tbody style="font-size: smaller; display: none;" > 
								<?= $memberview->DataMmeber() ?>
							</tbody>
						</table>
					</div>
		...
		<!-- Modal -->
			<?php  
			$datas = $memberData->memberDb();
			foreach($datas['data'] as $data){
			?>
			<div class="modal fade" id="membercb<?= $data['username_member'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-scrollable">
					<div class="modal-content" style="background-color: rgb(48, 48, 48)">
						<div class="modal-header text-white">Current Balance <?= $data['username_member'] ?></div>
							<div class="modal-body">
								<?php  
								$coinnamee = array("DOGE","BTT","TRON","SHIBA");
								foreach($coinnamee as $coin){
									$cb = $memberData->getCurrentBlance($coin, $data['username_member']);
								?>
								<div class="mb-3">
									<label for="" class="form-label text-white"><?= $coin ?></label>
									<input type="text" name="cbmember[]" class="form-control" value="<?= $cb[0]["jumlah_saldo_cb"] ?>" readonly>
								</div>
								<?php } ?>
							</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Close</button>
							<!-- <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Submit</button> -->
						</div>
					</div>
				</div>
			</div>
			<div class="modal fade" id="member<?= $data['username_member'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-scrollable">
					<form action="" method="post" class="modal-content" style="background-color: rgb(48, 48, 48)">
						<div class="modal-header text-white">Edit Data <?= $data['username_member'] ?></div>
							<div class="modal-body">
								<div class="mb-3">
									<input type="hidden" name="username_member" value="<?= $data['username_member'] ?>">
									<label for="" class="form-label text-white">Status</label>
									<select name="status_member" id="" class="form-select">
										<?php  
										$options = array("Aktif","Tidak Aktif");
										foreach($options as $opt){
											$selected = $opt == $data['status_member'] ? 'selected="selected"' : '';
										?>
								...
								<div class="mb-3">
									...
									<input required type="email" name="email" id="" class="form-control" value="<?= $data['email_member'] ?>">
								</div>
								<div class="mb-3">
									<label for="" class="form-label text-white">No Telpn</label>
									<input required type="number" name="notelpn" id="" class="form-control" value="<?= $data['telpn_member'] ?>">
								</div>
								<div class="mb-3">
									<label for="" class="form-label text-white">Password</label>
									<input required type="password" name="password" id="pass" class="form-control">
								</div>
								<div class="mb-3">
									<label for="" class="form-label text-white">Confirm Password</label>
									<input required type="password" id="passconf" class="form-control">
								</div>
							</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Close</button>
							<button type="submit" name="edit_member" class="btn btn-success btn-sm">Submit</button>
						</div>
					</form>
				</div>
			</div>
