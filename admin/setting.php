...
<html lang="en">
	...
	<body>
		...
		<form method="post" action="" class="container">
            <div class="row">
                <h5 class="text-center text-white mt-5 mb-3" style="font-size: smaller">ALAMAT WALLET</h5>
                <div class="row mb-3">
                    <?php  
                    $wallet = $dataadmin->getAdmin();
                    foreach($wallet as $url){
                    ?>
                    <div class="col-6 col-sm-6">
                        <label class="form-label text-white"><?= $url['type_wallet'] ?></label>
                        <input type="text" name="walletadmin[]" id="" class="form-control" value="<?=  $url['url_wallet'] ?>">
                    </div>
                    <?php } ?>
                </div>
            </div>
            <button type="submit" name="simpan_wallet" class="btn btn-success">Simpan</button>
		</form>
        <?php if($_SESSION['username_admin']=="galeriide"){ ?>
        <form action="" method="post" class="container mb-3">
            <div class="row">
                <h5 class="text-center text-white mt-5 mb-3" style="font-size: smaller">BONUS LEVEL</h5>
                <div class="row mb-3">
                    <?php  
                    $daabonus = $dataadmin->getBonus();
                    foreach($daabonus as $lvl){
                    ?>
                    <div class="col-12 col-sm-12">
                        <label class="form-label text-white"><?= $lvl['level_bonus'] ?></label>
                        <input type="number" step="0.0001" name="bonusPersen[]" id="" class="form-control" value="<?=  $lvl['persen_bonus'] ?>">
                    </div>
                    <?php } ?>
                </div>
            </div>
            <button type="submit" name="simpan_bonus" class="btn btn-success">Simpan</button>
        </form>
        <?php } ?>
        <form action="" method="post" class="container mb-5">
            <div class="row">
                <h5 class="text-center text-white mt-5 mb-3" style="font-size: smaller">BIAYA ADMIN</h5>
                <div class="row mb-3">
                    <?php  
                    $adminby = $dataadmin->getBiayaAdmin();
                    foreach($adminby as $by){
                    ?>
                    <div class="col-6 col-sm-6">
                        <label class="form-label text-white"><?= $by['ket_biaya'] ?></label>
                        <input type="number" step="0.0001" name="persen[]" id="" class="form-control" value="<?=  $by['persen_biaya'] ?>">
                    </div>
                    <?php } ?>
                </div>
            </div>
            <button type="submit" name="biaya_aksi" class="btn btn-success mb-5">Simpan</button>
        </form>
